...
public class NotificationService {
    ...
    public void notifyApplicationStatus(Long userId,
        ...
        var n = Notification.builder()
                ...
                .title("Hồ sơ của bạn đã được công ty " +companyName+ " được cập nhật: " + status)
                .message("Trạng thái hồ sơ của bạn đã thay đổi.")
                .companyId(companyId)
                .jobId(jobId)
    ...
    public void notifyNewJobToFollowers(Long companyId, Long jobId, String companyName, String jobTitle) {
        ...
        for (Long uid : uids) {
            var n = Notification.builder()
                    .userId(uid)
                    .type("NEW_JOB")
                    .title("Công ty " + companyName + " đăng tuyển: " + jobTitle)
                    .message("Xem ở bảng tin.")
                    .companyId(companyId)
                    .jobId(jobId)
                    .createdAt(Instant.now())
                    .build();
